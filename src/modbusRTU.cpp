...
modbusRTU::modbusRTU(configManager &configs) : settings() {
    ...
    //return a modbus_t context object for modbus RTU backend r
    this->ctx = modbus_new_rtu(settings.DEVICE.c_str(), settings.BAUD, settings.PARITY, settings.DATABITS,
                               settings.STOPBITS);
    if (this->ctx == nullptr) {
        throw std::runtime_error("Failed to create modbus context");
    }
    //allocate memory for buffer readings
    this->buffer = new uint16_t[settings.REGEND - settings.REGSTART + 1];
}
...
void modbusRTU::readRegisters() {
    ...
    if (modbus_read_registers(this->ctx, settings.REGSTART, settings.REGEND, this->buffer) == -1) {
...
void modbusRTU::updatePemData(pemData &data) {
    ...
    //@Switch case, to easly only store the data we want from that intervall
    //@modbus_get_float pass address of the first of 2 addresses we want to combine into one float
    for (int i = settings.REGSTART; i <= settings.REGEND; i++) {
        switch (i) {
            case 0:
                data.voltageL1_V = modbus_get_float(&this->buffer[i]);
                break;
            case 2:
                data.voltageL2_V = modbus_get_float(&this->buffer[i]);
                break;
            case 4:
                data.voltageL3_V = modbus_get_float(&this->buffer[i]);
                break;
            case 16:
                data.currentL1_A = modbus_get_float(&this->buffer[i]);
                break;
            case 18:
                data.currentL2_A = modbus_get_float(&this->buffer[i]);
                break;
            case 20:
                data.currentL3_A = modbus_get_float(&this->buffer[i]);
                break;
            case 30:
                data.activePowerTotal_W = modbus_get_float(&this->buffer[i]);
                break;
            case 56:
                data.frequency_Hz = modbus_get_float(&this->buffer[i]);
                break;
            default:
